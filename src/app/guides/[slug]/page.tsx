...
export async function generateMetadata({ params }: PageProps<"/guides/[slug]">): Promise<Metadata> { const { slug } = await params; const guide = getGuide(slug); return guide ? createMetadata(guide.tdk.title, guide.tdk.description, `/guides/${guide.slug}`, { type: "article", image: null, keywords: guide.tdk.keywords }) : {}; }
