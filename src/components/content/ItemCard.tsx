  ...
  return (
    <article className={styles.itemCard}>
      <div className={styles.itemImage}>
        {item.image ? (
          <Image src={item.image} alt={artworkPending ? `${item.name} artwork pending` : `${item.name} official game icon`} width={72} height={72} unoptimized={item.image.endsWith(".gif") || item.image.startsWith("http")} />
        ) : (
