...
export default function UnitDetailPage({ unit }: { unit: Unit }) {
  ...
  return (
    <main className={`container ${styles.detailShell}`}>
      <Breadcrumb items={[{ label: "Home", href: "/" }, { label: "Units", href: "/wiki/units" }, { label: unit.faction, href: `/wiki/factions/${unit.factionSlug}` }, { label: unit.name }]} />
        ...
        <article className={styles.detailMain}>
          <header className={styles.unitHero} style={{ "--unit-accent": unit.accent } as CSSProperties}>
            ...
            <div className={styles.heroCopy}>
              ...
              <dl className={styles.statStrip}>{[["Health", unit.stats.hp], ["Attack", unit.stats.atk], ["Armor", unit.stats.ar], ["Critical", `${unit.stats.crt}%`], ["Speed", signed(unit.stats.spd, "%")], ["Range", unit.stats.rng]].map(([label, value]) => <div key={label}><dt>{label}</dt><dd>{value}</dd></div>)}</dl>
            </div>
          </header>

          <section className={styles.panel} id="overview"><h2>Overview</h2><div className={styles.overviewBody}><UnitSprite src={unit.image} color={unit.accent} /><p>{unit.summary} Its verified public card identifies <strong>{unit.tactic.name}</strong> as its battlefield tactic. The guidance on this page stays within that published behavior and labels strategy interpretation separately.</p></div></section>

          <section className={styles.panel} id="stats"><h2>Full Stats</h2><div className={styles.tableScroll}><table className={styles.statsTable}><thead><tr><th>ES</th><th>HP</th><th>MP</th><th>STR</th><th>AGI</th><th>INT</th><th>ATK</th><th>CRT</th><th>RNG</th><th>SPD</th><th>AR</th><th>EVA</th></tr></thead><tbody><tr><td>{unit.stats.es}</td><td>{unit.stats.hp}</td><td>{unit.stats.mp}</td><td>{unit.stats.str}</td><td>{unit.stats.agi}</td><td>{unit.stats.int}</td><td>{unit.stats.atk}</td><td>{unit.stats.crt}%</td><td>{unit.stats.rng}</td><td>{signed(unit.stats.spd, "%")}</td><td>{unit.stats.ar}</td><td>{unit.stats.eva}%</td></tr></tbody></table></div><p>Baseline public-card values for {unit.gameVersion}; traits, equipment, leaders, and in-run progression can alter the result.</p></section>
          ...
          <section className={styles.panel}><h2>Related Guides</h2><div className={styles.relatedGrid}>{relatedUnits.map((entry) => <Link key={entry.slug} href={`/wiki/units/${entry.slug}`}><b>{entry.name}</b><span>{entry.trait.name} · {entry.tactic.name}</span></Link>)}<Link href="/guides/formation-guide"><b>Formation Guide</b><span>Positioning and flanking fundamentals.</span></Link><Link href="/guides/stats-explained"><b>Stats Explained</b><span>Read every value on the card.</span></Link></div></section>
        </article>
